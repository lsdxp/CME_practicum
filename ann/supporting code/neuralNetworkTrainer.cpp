...
void neuralNetworkTrainer::trainNetwork( trainingDataSet* tSet )
{
    ...
    validationSetAccuracy = NN->getVSetAccuracy(tSet->validationSet);
...
void neuralNetworkTrainer::runTrainingEpoch( vector<dataEntry*> trainingSet )
{
    ...
    double mse = 0;
    vector<vector<double>> trainingsetresult;
    ...
    for ( int tp = 0; tp < (int) trainingSet.size(); tp++)
    {
        ...
        //log training results
        vector<double> temp;
        trainingsetresult.push_back(temp);
        trainingsetresult[tp].push_back(NN->outputNeurons[0]);
        // trainingsetresult[tp].push_back(NN->outputNeurons[1]);
        // trainingsetresult[tp].push_back(NN->outputNeurons[2]);
    ...
    fstream logtrainingset;
    if (!logtrainingset.is_open())
    {
        logtrainingset.open("training_set_result.csv", ios::out);
        
        if ( logtrainingset.is_open() )
        {
            //write log file header
            for(int i = 0; i < trainingsetresult.size(); i++){
                logtrainingset << trainingsetresult[i][0]
                // << "," << trainingsetresult[i][1] << "," << trainingsetresult[i][2]
                << endl;
            }
        }
    }
