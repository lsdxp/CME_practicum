...
bool dataReader::loadDataFile( const char* filename, int nI, int nT )
{
    ...
    if ( inputFile.is_open() )
    {
        ...
        if (!logFile.is_open())
        {
            logFile.open("shuffled_data.csv", ios::out);
            
            if ( logFile.is_open() )
            {
                //write log file header
                for(int i = 0; i < data.size(); i++){
                    logFile << data[i]->pattern[0] << "," << data[i]->pattern[1] << "," <<  data[i]->pattern[2] << "," <<
                    data[i]->pattern[3] << ","<<data[i]->target[0]
                    // << "," << data[i]->target[1] << ","<< data[i]->target[2]
                    << endl;
                }
            }
        }
